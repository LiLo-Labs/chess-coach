...
/// Reads a polyglot .bin opening book file.
/// Format: sorted array of 16-byte entries (8B Zobrist hash + 2B move + 2B weight + 4B learn).
struct PolyglotBook: Sendable {
    ...
    init(data: Data) {
        self.data = data
        self.entryCount = data.count / Self.entrySize
    }

    init?(bundleResource name: String, extension ext: String = "bin") {
    ...
    struct BookEntry: Sendable {
        let hash: UInt64
        let move: PolyglotMove
        let weight: UInt16
        let learn: UInt32
    }

    struct PolyglotMove: Sendable, Equatable {
        let raw: UInt16

        var toFile: Int { Int(raw & 0x7) }
        var toRow: Int { Int((raw >> 3) & 0x7) }
        var fromFile: Int { Int((raw >> 6) & 0x7) }
        var fromRow: Int { Int((raw >> 9) & 0x7) }
        var promotion: Int { Int((raw >> 12) & 0x7) }

        /// Convert to UCI string (e.g., "e2e4").
        var uci: String {
            let fromStr = "\(Character(UnicodeScalar(97 + fromFile)!))\(fromRow + 1)"
            let toStr = "\(Character(UnicodeScalar(97 + toFile)!))\(toRow + 1)"

            // Handle castling: polyglot encodes king captures rook
            if fromStr == "e1" && toStr == "h1" { return "e1g1" }
            if fromStr == "e1" && toStr == "a1" { return "e1c1" }
            if fromStr == "e8" && toStr == "h8" { return "e8g8" }
            if fromStr == "e8" && toStr == "a8" { return "e8c8" }

            var result = fromStr + toStr
            if promotion != 0 {
                let promoChars = ["", "n", "b", "r", "q"]
                if promotion < promoChars.count {
                    result += promoChars[promotion]
                }
            }
            return result
        }
    ...
    /// Look up all entries for a position given its Zobrist hash.
    func lookup(hash: UInt64) -> [(move: String, weight: UInt16)] {
        ...
        // Find leftmost entry with matching hash
        var firstIndex = -1
        while low <= high {
            let mid = (low + high) / 2
            let entry = readEntry(at: mid)
            if entry.hash < hash {
                low = mid + 1
        ...
        // Collect all entries with this hash (they're contiguous since file is sorted)
        var results: [(move: String, weight: UInt16)] = []
        var i = firstIndex
        while i < entryCount {
            let entry = readEntry(at: i)
            guard entry.hash == hash else { break }
            results.append((move: entry.move.uci, weight: entry.weight))
            i += 1
        }
    ...
    private func readEntry(at index: Int) -> BookEntry {
        let offset = index * Self.entrySize
        let hash = data.readBigEndianUInt64(at: offset)
        let moveRaw = data.readBigEndianUInt16(at: offset + 8)
        let weight = data.readBigEndianUInt16(at: offset + 10)
        let learn = data.readBigEndianUInt32(at: offset + 12)
        return BookEntry(
            hash: hash,
            move: PolyglotMove(raw: moveRaw),
            weight: weight,
            learn: learn
        )
    }
...
// MARK: - Polyglot Zobrist Hashing

/// Computes Zobrist hashes matching the polyglot specification.
/// Uses the standard 781 random numbers defined in the polyglot spec.
enum PolyglotZobrist {
    /// Compute the polyglot Zobrist hash for a FEN position.
    static func hash(fen: String) -> UInt64 {
        ...
        for (rankIdx, row) in rows.enumerated() {
            let rank = 7 - rankIdx  // FEN starts from rank 8
            var file = 0
            for ch in row {
                if let skip = ch.wholeNumberValue {
                    file += skip
                } else {
                    if let pieceIndex = pieceIndex(for: ch) {
                        h ^= randomArray[64 * pieceIndex + 8 * rank + file]
                    }
                    file += 1
                }
            }
        ...
        // Castling
        if castling.contains("K") { h ^= randomArray[768] }
        if castling.contains("Q") { h ^= randomArray[769] }
        if castling.contains("k") { h ^= randomArray[770] }
        if castling.contains("q") { h ^= randomArray[771] }

        // En passant (only if a pawn can actually capture)
        if enPassant != "-" && enPassant.count == 2 {
            let epFile = Int(enPassant.first!.asciiValue!) - 97
            if epFile >= 0 && epFile < 8 {
                // Polyglot only includes en passant if capture is possible
                // For simplicity, always include it (matches most book generators)
                h ^= randomArray[772 + epFile]
            }
        }
    ...
    /// Map piece character to polyglot piece index (0-11).
    /// Order: BlackPawn, WhitePawn, BlackKnight, WhiteKnight, BlackBishop, WhiteBishop,
    ///        BlackRook, WhiteRook, BlackQueen, WhiteQueen, BlackKing, WhiteKing
    private static func pieceIndex(for ch: Character) -> Int? {
        switch ch {
        case "P": return 1
        case "p": return 0
        case "N": return 3
    ...
    // MARK: - The 781 random numbers from the polyglot specification

    static let randomArray: [UInt64] = [
        // Piece keys: 12 pieces × 64 squares = 768 values
        // Values from the public domain polyglot specification
        // piece_index * 64 + rank * 8 + file
        // BlackPawn (index 0)
        ...
        0x31D71DCE64B2C310,  // 768: white kingside
        0xF165B587DF898190,  // 769: white queenside
        0xA57E6339DD2CF3A0,  // 770: black kingside
        0x1EF6E6DBB1961EC9,  // 771: black queenside
        // En passant keys (772-779, one per file a-h)
        0x70CC73D90BC26E24,  // 772: a-file
        0xE21A6B35DF0C3AD7,  // 773: b-file
        0x003A93D8B2806962,  // 774: c-file
        0x1C99DED33CB890A1,  // 775: d-file
        0xCF3145DE0ADD4289,  // 776: e-file
        0xD0E4427A5514FB72,  // 777: f-file
        0x77C621CC9FB3A483,  // 778: g-file
        0x67A34DAC4356550B,  // 779: h-file
        // Turn key (780)
        0xF8D626AAAF278509,  // 780: side to move (XOR when white to move)
    ]
}
